...
function loadSearch(){
    var innerHTML = '';
    for (var j=0; j<hotGames.length; j++) {
        var item = hotGames[j];
        var img = item.img;
        var slug = "/game/"+item.slug+".html";
        if(item.slug == main_slug){
        ...
        var html = `<a class="m-2 game-icon-a group w-[94px]" href="${slug}">
            ...
            <h3 class="game-icon-h3">${item.title}</h3>
        </a>`;
        innerHTML += html;
    }
    document.querySelector('#searchHot').innerHTML = innerHTML;
}
function loadGA(){
